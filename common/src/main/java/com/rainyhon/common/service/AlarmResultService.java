...
import com.rainyhon.common.model.AlarmResultExample;
...
public class AlarmResultService {
	...
	public PageInfo<AlarmResultVo> getRealAlarmList(String alarmType, String operator, String state,
		...
		PageInfo<AlarmResult> realAlarmPageInfo = PageHelper.startPage(pageIndex, pageSize)
				.doSelectPageInfo(() -> alarmResultMapper.selectByExample(example));
	...
	public ResultVo delete(String id) {
		String[] ids = id.split(",");
		for (String i : ids) {
			alarmResultMapper.deleteByPrimaryKey(i);
		}
	...
	public ResultVo add(AlarmResultVo vo) {
		...
		alarmResultMapper.insert(realAlarm);
	...
	public ResultVo getAlarmCount() {
		String sql = "SELECT alarm_type, count( * ) count, p.para_value " +
				"FROM alarm_result " +
				"LEFT JOIN para_define p ON alarm_type = p.para_code AND p.para_type = 'ALARM_MODE_TYPE' " +
				"GROUP BY alarm_type;";
